...
an `en.json` change without the matching `pl.json`/`zh.json` entries. This is
not a convention the runtime needs — missing values silently fall back to
English — it is how a locale avoids rotting, and `tests/i18n.test.js` enforces
it on every `npm test` run (CI included):
...
- **Placeholder coverage** — every `{input}` in the English message must appear
  ...
  rephrasing, e.g. Polish "równa {root} … poddrzewie {root}"); dropping one is
  not — Paraglide would render a hole at runtime. Matcher messages (array
  values) are exempt: locales legitimately prune plural branches (zh keeps
  only `*`; pl keeps `one/few/many/*`).

Workflow when adding or changing a message:

1. Add/modify the key in `messages/en.json` (with the `$schema` line kept).
2. Add the same key, in the same position, to `pl.json` and `zh.json`.
...
概览/供应商/模型/游乐场/审计日志/用量/预算/限流/API 密钥/护栏/工作流/MCP 服务器/
用户/设置/主题(浅色/深色)/语言/看板(dashboard)。`token`→`令牌`;`rate limit`→`限流`;
`key`→`密钥`;`failover`→`故障转移`;`alias`→`别名`;`API key`→`API 密钥`;
`Base URL`→`基础 URL`。Verbatim technical tokens (matches the Polish precedent):
endpoint names Chat Completions/Responses/Messages, JSON, `HTTP {status}`, Slug.
