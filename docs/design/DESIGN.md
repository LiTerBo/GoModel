---
version: alpha
name: gomodel-dashboard-design
description: "Design language of the GoModel Dashboard — an AI gateway management panel. Warm-toned, dark-first UI: near-black canvas #111110 with layered warm-gray surfaces, a single gold accent #b8956e for primary actions, and a four-stop semantic status palette (green/blue/orange/red) mapped to provider/rate-limit states. Theme parity via data-theme + CSS custom properties with a 3-state toggle (light/system/dark). System font stacks at restrained 400-700 weights. Signature components: the collapsible resizable sidebar (240↔60 px), the live token-throughput chart panel, and the provider-configuration editor."

colors:
  # canvas & surfaces (warm, dark-first)
  bg: "#111110"
  surface: "#1e1d1c"
  ...
  light-surface-hover: "#ece5dc"
  light-border: "#e8e0d6"
  light-text: "#2d2519"
  light-text-muted: "#7a7068"
  light-accent: "#755c3d"
  light-accent-hover: "#9a7d5a"
  light-info: "#2563eb"
  light-warning: "#d97706"
  light-danger: "#dc2626"
  # token throughput colours (AI gateway specific)
  token-input: "#c0824a"
  token-output: "#ddb27a"
...
elevation:
  ...
  level-3: "0 10px 22px rgba(59,130,246,0.18)"
...
The surface reads as a professional operations tool: **warm-toned cards on a near-black canvas** (`{colors.bg}` `#111110`) for the default dark theme, separated by `{colors.border}` `#2a2826` hairlines. Around this neutral-warm base, the brand layers a **single gold accent** `{colors.accent}` `#b8956e` reserved for primary actions, active states, links, and progress fills; a four-stop semantic status palette maps to gateway states: green `{colors.green}` (healthy/success), blue `{colors.blue}` (processing/active), orange `{colors.orange}` (warning/rate-limited), red `{colors.red}` (error/down).

Type uses `Inter` with system fallbacks (`-apple-system` / `BlinkMacSystemFont` / `Segoe UI` / `Roboto`) at restrained weights — 400 for body, 500 for navigation items, 600 for buttons and labels, 700 for page headers and statistics values. The UI guarantees **WCAG 2.1 AA contrast (≥ 4.5:1)** — dark text `{colors.text}` `#e8e0d6` on warm surfaces for dark theme, warm dark `{colors.light-text}` `#2d2519` on white cards for light theme.

The shape system is unified: `{rounded.md}` `8px` is the single radius used for cards, inputs, modals, and nav items; `{rounded.sm}` `6px` is reserved for buttons; `{rounded.full}` for circular elements. The `{rounded.lg}` `10px` variant is used for badges. Elevation is minimal — `{elevation.level-1}` hairline borders are the default chrome, with `{elevation.level-3}` accent-tinted shadow for the primary CTA button.

**Key Characteristics:**
- A **warm-toned, dark-first canvas** — near-black surfaces with warm-gray borders; a single gold accent for every primary action.
...
### Brand & Accent
- **Accent Gold** (`{colors.accent}` — `#b8956e`): The brand's single conversion colour — primary CTAs, active states, links, focus rings, progress fills, logo colour.
- **Accent Hover** (`{colors.accent-hover}` — `#d4b896`): Hover state for primary buttons and active nav items.

### Surface (Dark Theme — Default)
- **Canvas** (`{colors.bg}` — `#111110`): The default page background (dark, warm near-black).
- **Surface** (`{colors.surface}` — `#1e1d1c`): Cards, panels, sidebar, modals, table wrappers, chart containers.
- **Surface Hover** (`{colors.surface-hover}` — `#2a2420`): Table row hover, nav item hover, button hover, dropdown items.
- **Border** (`{colors.border}` — `#2a2826`): 1 px solid borders — card chrome, input borders, dividers, table row separators.

### Surface (Light Theme — Explicit & System)
- **Canvas** (`{colors.light-bg}` — `#f5f0ea`): The light page background, a warm cream.
- **Surface** (`{colors.light-surface}` — `#ffffff`): White cards, panels, modals.
- **Surface Hover** (`{colors.light-surface-hover}` — `#ece5dc`): Light hover state.
- **Border** (`{colors.light-border}` — `#e8e0d6`): Light hairline borders.

### Text
- **Text** (`{colors.text}` — `#e8e0d6`): Default text and headings (dark theme). Light theme: `{colors.light-text}` `#2d2519`.
- **Text Muted** (`{colors.text-muted}` — `#9a918a`): Secondary text — meta, captions, placeholders, disabled, table headers. Light theme: `{colors.light-text-muted}` `#7a7068`.

### Semantic Status
- **Success Green** (`{colors.green}` — `#34d399`): Healthy states, active models, successful operations, active auth-key status badges.
- **Info Blue** (`{colors.blue}` — `#3b82f6`): Processing states, prompting cache indicator, calendar heatmap base. Light theme: `{colors.light-info}` `#2563eb`.
...
- **Error Red** (`{colors.red}` — `#ef4444`): Error states, destructive actions, inactive/failed status. Light theme: `{colors.light-danger}` `#dc2626`.
...
|-------|--------|-------|-----------|
| Input | `{colors.token-input}` | `#c0824a` | Paid input tokens — solid warm brown |
| Output | `{colors.token-output}` | `#ddb27a` | Paid output tokens — lighter warm brown |
...
- **Dark theme**: climbs from `#161b22` (dark navy, level 0) through `--info` `#3b82f6` (level 8) to a bright tint (level 10).
- **Light theme**: climbs from `#ebedf0` (pale gray, level 0) through `--info` `#2563eb` (level 7) to a deep navy (level 10).
...
| Level 3 — Accent Lift | `{elevation.level-3}` — `0 10px 22px rgba(59,130,246,0.18)`. | Primary CTA button (`btn-primary`). |
| Level 4 — Danger Lift | `{elevation.level-4}` — `0 10px 22px color-mix(in srgb, red, 20%, transparent)`. | Danger button (`btn-danger`). |

### Decorative Depth
Depth comes from the layered warm surfaces and the gold accent — no gradients, no photography, no background patterns. The sole decorative flourish is the `color-mix()` accent tint on the primary CTA shadow.
...
**`btn-primary`** — the canonical gold CTA.
- Background `{colors.accent}`, text white, `{typography.button-strong}` (13 px 600), `{rounded.sm}`, `{elevation.level-3}` accent-tinted shadow. Hover darkens via `color-mix(in srgb, var(--accent) 90%, #fff 10%)`.
...
### Do
- Reserve `{colors.accent}` (`#b8956e`) for primary actions, active states, links, and progress fills. The accent is the only conversion colour.
...
### Don't
- Don't use chromatic accents as button backgrounds — gold is the only primary action colour.
- Don't promote body weight beyond 400; keep the ceiling at 700 (page headers and statistics only).
- Don't break the warm-toned palette with cool-grey surfaces — the warm colour temperature is a brand identifier.
...
- Don't use gradients for any element — the design has no gradient gradient decoration.
