...
规则：节点不写 `allowed_models` = 全部模型可用；一旦写了，该路径下的请求被限制在本节点**及所有父节点**列表的交集内。单个名单内按「**请求名** ∨ 解析后目标」判定：条目可写 `provider/*`、`provider/model`、裸模型 ID，**也可写虚拟模型名**（如 `smart`）——按名字授权后，该路径下的使用方只需认这个名字，不必知道它背后是哪些真实模型，`/v1/models` 也只列这个名字且不带目标供应商/时间痕迹；写具体目标时语义与以前一致（多目标别名仍要求每个可能命中的目标都被允许）。
