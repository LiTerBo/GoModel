...
when the policy is saved. An entry may also be the name of a **virtual model**
(such as `smart`): a request asking for that name is authorized by the name
itself, so the caller never has to know which concrete models sit behind it.
Inside one list an entry admits the request when it matches either the
requested name or the resolved target selector; allowlists still intersect
along the chain, so a child never widens its parent.

Because the name is what gets authorized, a key whose list names an alias sees
just that alias in `GET /v1/models` and can call it whatever targets it
currently has — including a load balancer whose targets span providers, which
used to fail with `400` whenever it picked a disallowed target. The alias is
listed without target metadata (`owned_by` empty), so the caller cannot infer
the provider or the concrete model behind it. The dashboard pickers on the
**API Keys** and **Users** pages list those names next to the provider
wildcards and concrete models, and every allowlist field still accepts typed
entries. In the picker, each alias bears a **follows alias** badge; hovering
it reveals the alias's current target models. Entries naming concrete targets
keep their old meaning: for such a list a load balancer is still only usable
when every target it can pick is allowed, and a node whose only entry is a
virtual model name shows an empty effective list in the user tree.
...
list, and model-side policies combined, with virtual models listed under the
name a request would use.
