...
On the **Models** page, a `*` next to a price marks a model with time windows; hover it to see the schedule and rates. From the **Pricing override** editor you can now add, edit, and remove time windows alongside the base rates — open any model's override, then use the **Add Window** button. `config.yaml` model metadata may also declare its own `time_windows`, which replace the catalog's.
...
You can also add **time windows** — recurring UTC schedules with discounted rates. Use the **Add Window** button below the scalar fields, then set a label (e.g. `off_peak`), one or more UTC ranges, and the discounted rates for each window. Each window supports multi-day selection (mon–sun), start/end times in `HH:MM` format, and separate input, output, and cached-input rates. The operator's scalar base prices stay as the peak (standard) rates; the window's rates are consulted at request time via [`AtTime`](/features/cost-tracking#time-of-day-pricing).
