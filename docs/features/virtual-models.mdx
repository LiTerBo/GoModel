...
A shadowed model row offers **no access switch**. That switch writes an access
policy under the row's own selector, and the selector is the redirect's `source`
— the primary key — so the write would replace the redirect with a policy and
drop its pointing. Use the row's **Edit redirect** action to change the
pointing, or **Remove redirect** to retire the shadow on purpose; the admin API
asks for the same explicit intent as `clear_targets: true`.

Deleting an alias that still has API keys or user paths authorized to reach it
returns `409 Conflict` with an explanation of what references it: the dashboard
renders that prompt in the console language (the API itself always answers in
English). A **force delete** (`force: true` in the admin API or the
corresponding dashboard action) bypasses the guard and removes the alias
regardless of outstanding authorizations. The guard covers redirects only —
access policies resolve no name, so removing one (how the Models page puts a
single model back on the shelf) is never gated. A **paused** redirect
(`enabled: false`) is not gated either: requests aimed at it are refused before
routing, so nobody reaches anything through it and the delete takes nothing
away. Turn it back on if you want the guard to describe it again.

## Lock an alias

The redirect editor's **Lock** toggle prevents accidental target changes. When
locked, saving a new pointing returns `409 Conflict` with a message explaining
that the alias is locked. To change the target, unlock first — either uncheck
the toggle, or use the **retarget-and-unlock** gesture that changes the
pointing and unlocks in one action. The lock does not block deletion,
description edits, or the enabled/disabled toggle.

## Preview who this alias affects

Before saving target changes, the editor shows who reaches this alias:
- **Follow** — names authorized by name to reach this alias.
- **Potential** — names authorized by a shared selector (user path) — they
  match the alias and *may* be affected.

Both lists come from the `authorized-by` endpoint.
