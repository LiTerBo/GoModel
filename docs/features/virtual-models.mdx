...
— one key, one row — so the switch would address the alias instead of the
concrete model the row shows. Change the alias from its own row (**Edit
redirect**), or retire it there with **Remove redirect**, which sends the same
explicit `clear_targets` intent the admin API asks for.
