...
1. **同类英文提示未扫全**：`internal/` 内 `WithCode(...)`（另有 1 个以 `code = "..."` 常量出现）共 **24 个**机器码，前端原来只渲染 2 个。**已收口（2026-09-13）**：见 `docs/dev/2026-09-13_api-error-code-localization.md`，[#54](https://github.com/LiTerBo/GoModel/issues/54) 已关闭（单表 `$lib/api/errors.js` + 双向覆盖测试）。
...
3. **禁用状态的别名**：守卫对 `enabled=false` 的 redirect 仍会拦（严格说此时无人可达）。**已收口（2026-09-13）**：见本文件 §7，[#56](https://github.com/LiTerBo/GoModel/issues/56) 已关闭。
...
## 7. 收口 #56：已下架的 redirect 删除不再被守卫拦

- **现象**：`enabled=false` 的 redirect（已下架别名）仍带授权凭据时，`DELETE` 返回 `409 virtual_model_in_use`「is still reachable by 1 credential(s)」——可这条别名根本不可达，提示与事实不符，且不点「强制」就删不掉。
- **根因**：守卫条件只看 `len(stored.Targets) > 0`（是 redirect 就查可达清单），没看行自身的 `enabled`。而解析路径在授权阶段就拒了：`virtualmodels.AllowsModel` / `ValidateModelAccess` 先判 `!state.Enabled` ⇒ 400 `model_access_denied`，且快照里行的 `Enabled` 对该 selector 是权威值（`effectiveState` 命中规则时 `state.Enabled = rule.Enabled`）。
- **修复**：`internal/admin/handler_virtualmodels.go` 守卫条件收为 `len(stored.Targets) > 0 && stored.Enabled`（与既有「访问策略从不守卫」同一处判断，注释并列说明两种「不解析任何名字」的情形）。
- **验证**：新增 `TestDeleteVirtualModelPausedRedirectNeedsNoForce`（同一 fixture 下两个子用例：`enabled=false` 的 redirect 删得掉且行消失；`enabled=true` 的同形行仍 409 `virtual_model_in_use` 且行保留）——先红（红=409，日志复现了那句「still reachable by 1 credential(s)」）后绿。既有守卫用例（按名/按 target 可达仍拦、force 才放行、策略行不需 force、不受限持有者仍拦）全部保持绿。
- **前端**：无需改动 —— 删除流程完全由服务端 409 驱动（前端无客户端预判、无 `enabled` 相关特判），本修复后已下架行直接删成功、不再弹「强制」对话框。
- **文档**：`docs/advanced/admin-endpoints.mdx` 新增「Paused redirects are never gated.」段；`docs/features/virtual-models.mdx` 删除守卫段补同一句。
