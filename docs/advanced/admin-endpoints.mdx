...
**The `clear_targets` field.** A virtual model whose `source` is already a
redirect (it has a `target_model` or `targets`) rejects a body that carries no
pointing: `source` is the primary key, so accepting it would replace the alias
definition with an access policy and drop the pointing. Send
`clear_targets: true` to make that kind change on purpose — the row keeps its
`description`, `user_paths`, `slowdown`, and `enabled`, and leaves the routing
table. A redirect writing a redirect, and a policy turning into a redirect,
need no gesture. Only a request that *removes* a stored pointing does.
