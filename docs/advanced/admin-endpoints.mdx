...
models a request with that key can actually call, with its user path
policies, its own allowlist, and model-side policies applied. Virtual models
the key may address are listed under their own name, so the column matches
what `GET /v1/models` returns for that key.
...
`PUT /admin/auth-keys/<key-id>/allowed-models` (`{"allowed_models": ["anthropic/*", "openai/gpt-4o", "smart"]}`;
an explicit empty list lifts the key-level restriction; omitting the field is rejected). An entry may be a
**virtual model name** (`smart` in the example): it authorizes requests that ask for that name, so the key can
call the alias whatever targets it has and `GET /v1/models` lists just that alias, without the provider or
concrete model behind it. Group- and user-level
...
authorizer inference uses, with virtual models the path may address listed by
name; a managed API key's own `allowed_models` may narrow
...
Virtual models are managed under `/admin/virtual-models`. These endpoints
require the global admin scope (regular user-path scoped credentials get
`403 admin_scope_denied`).

| Method   | Path                                              | Description                                                    |
| -------- | ------------------------------------------------- | -------------------------------------------------------------- |
| `GET`    | `/admin/virtual-models`                           | List virtual models (redirects and access policies)            |
| `PUT`    | `/admin/virtual-models`                           | Create, update, or rename one virtual model                    |
| `DELETE` | `/admin/virtual-models`                           | Delete one virtual model                                       |
| `GET`    | `/admin/virtual-models/authorized-by`             | Impact preview: who a proposed change reaches                  |

#### `GET /admin/virtual-models`

Lists every virtual model — both redirects (aliases that point at concrete
models) and access policies (per-virtual-model allow-lists that restrict which
user paths or API keys can reach it).
...
**Error codes:** `401` (unauthenticated), `503` (feature unavailable).

#### `PUT /admin/virtual-models`

Creates, updates, or renames one virtual model. Send `old_source` to rename.
Fields omitted from the body keep their stored value.

**Request body:**

| Field               | Type                | Description                                                        |
| ------------------- | ------------------- | ------------------------------------------------------------------ |
| `source`            | `string`            | **Required.** Virtual model name (alias)                           |
| `old_source`        | `string`            | Previous name — set to trigger a rename                            |
| `target_model`      | `string`            | Single redirect target (e.g. `gpt-4o`) — mutually exclusive with `targets` |
| `targets`           | `array`             | Load-balancing destinations: `[{provider, model, weight?}]`        |
| `strategy`          | `string`            | `round_robin`, `cost`, `failover`, `adaptive`, or `plugin`         |
| `strategy_plugin`   | `string`            | Routing-strategy plugin name (when `strategy` is `plugin`)         |
| `strategy_config`   | `object`            | Per-virtual-model plugin configuration                             |
| `session_affinity`  | `bool`              | Keep requests of one session on the same target (omitted = enabled)|
| `failover`          | `bool`              | Retry failed request on remaining targets (omitted = enabled)      |
| `user_paths`        | `string[]`          | Restrict the alias to these user paths                             |
| `description`       | `string`            | Human-readable label                                               |
| `slowdown`          | `float`             | Extra-time factor 0.1–10 (zero disables it)                        |
| `enabled`           | `bool`              | Whether requests can reach this alias                              |
| `locked`            | `bool`              | Freezes the pointing (see below)                                   |
| `unlock`            | `bool`              | Explicit gesture to edit a locked row while keeping it locked      |
| `clear_targets`     | `bool`              | Explicit gesture to replace a stored redirect with an access policy|

**The `locked` field.** A locked virtual model rejects writes that change the
pointing — the targets, their weights and order, and the strategy that picks
among them. Edits to the description, user paths, slowdown, or enabled flag
are still accepted. To change the pointing of a locked row, send
`unlock: true` (keeps the locked state) or `locked: false` (turns locking
off) in the same request.

**Response `200`:** The updated virtual model as shown in `GET /admin/virtual-models`.

**The `clear_targets` field.** A request that asks for a row with no pointing — an
explicit empty `targets: []` — would replace a stored redirect's definition and
drop it: `source` is the primary key, so the row would silently stop being an
alias. Send `clear_targets: true` to do that on purpose; the row leaves the
routing table and keeps whatever else the request carries. A redirect writing a
redirect, and a policy turning into a redirect, need no gesture.

**Metadata-only writes keep the row.** A body that says nothing about the
pointing (no `target_model`, no targets) is a metadata edit: `locked`, `enabled`,
`description`, `user_paths`, and the stored pointing all keep their stored
values, so an API caller can flip one field without restating the whole alias.
A body that sends its own pointing replaces the rest of the row instead — that
is how an access policy becomes a redirect — and `enabled`/`locked` are the
tri-state fields that always keep their stored value when omitted.

**Response `204`:** Returned when the upsert produces no visible row (access
policy removed).
...
| Code | Description                                                        |
| ---- | ------------------------------------------------------------------ |
| `400`| Invalid input — missing source, malformed targets, validation error|
| `401`| Unauthenticated                                                    |
| `409`| **`virtual_model_locked`** — resolution config change on a locked row without `unlock` |
| `409`| **`virtual_model_kind_change`** — the body would replace a stored redirect with an access policy without `clear_targets` |
| `502`| Store failure                                                      |
| `503`| Feature unavailable                                                |

#### `DELETE /admin/virtual-models`

Removes one virtual model. The body identifies it by `source`.
...
| Field    | Type     | Description                                                        |
| -------- | -------- | ------------------------------------------------------------------ |
| `source` | `string` | **Required.** Virtual model name to remove                         |
| `force`  | `bool`   | Override the reachability guard (see below)                        |

**The `force` parameter.** Deleting a **redirect** (a row with targets) takes the
addressable name away from everyone reaching it: when credentials or user-path
policies still reach it, the delete is rejected with `409 virtual_model_in_use`
naming the counts. Set `force: true` to override the guard and take the alias
away from the listed holders. Use `GET /admin/virtual-models/authorized-by?source={name}`
to preview who will lose access before force-deleting.

**Access policies are never gated.** A row without targets resolves no name for
anybody, so deleting it takes nothing away — it drops the row's restriction and
restores the catalog default (the same gesture the Models page switch uses to
put a single model back on the shelf). Such a delete always answers `204`, even
when unrestricted credentials or user paths would match the reachability tally.

**Paused redirects are never gated.** A redirect with `enabled: false` resolves
no name either: a request aimed at it is refused before routing, so the holders
the tally lists reach nothing through it and the delete takes nothing away. The
delete answers `204` without `force`. Turn the row back on (`enabled: true`)
first if you want the guard to describe it again.
...
| Code | Description                                                        |
| ---- | ------------------------------------------------------------------ |
| `400`| Invalid input — missing source                                     |
| `401`| Unauthenticated                                                    |
| `404`| Virtual model not found                                            |
| `409`| **`virtual_model_in_use`** — credentials or user paths still reach the model; send `force` |
| `502`| Store failure                                                      |
| `503`| Feature unavailable                                                |

#### `GET /admin/virtual-models/authorized-by`

Impact preview endpoint that reports which policy holders (API keys and
user-path policies) a virtual model reaches. With `new_targets`, it evaluates
a proposed edit before saving, so the operator can see who would gain or
lose access.
...
| ------------- | -------- | -------- | --------------------------------------------------------------- |
| `source`      | `string` | Yes      | Virtual model source (alias name)                                |
| `new_targets` | `string` | No       | Comma-separated proposed targets (`provider/model`); defaults to stored targets |
...
```json
{
  ...
  "new_targets": ["anthropic/claude-sonnet-4"],
  "grants": [
    {
      "kind": "credential",
      "id": "ak_abc123",
...
Each grant's `change` field classifies the impact:

| Value            | Meaning                                                          |
| ---------------- | ---------------------------------------------------------------- |
| `follow`         | Authorized by the alias name itself; change is invisible to this holder |
| `potential`      | Authorized through target selectors that intersect the change; may gain or lose access |
| `unrestricted`   | No model allowlist; the alias is always available to this holder |
| (omitted)        | Holder that the change does not reach — not listed               |

**Error codes:**

| Code | Description                                                        |
| ---- | ------------------------------------------------------------------ |
| `400`| Missing or invalid `source`, or malformed `new_targets`            |
| `404`| Virtual model not found                                            |
| `503`| Feature unavailable                                                |
