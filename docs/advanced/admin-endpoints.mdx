...
**The `force` parameter.** When credentials or user-path policies still reach
the virtual model, the delete is rejected with `409 virtual_model_in_use`
naming the counts. Set `force: true` to override the guard and take the
alias away from the listed holders. Use `GET /admin/virtual-models/authorized-by?source={name}`
